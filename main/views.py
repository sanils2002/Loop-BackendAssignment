...
class StoreViewSet(ModelViewSet):
    ...
    def trigger_report(self, request,pk=None):
        if pk is not None:
            # If store ID is provided in the request, use it
            ...
            report_f = trigger_report_combined(report)
            return Response(status=200, data={"report_id": report_f.id})


    # @action(detail=True, methods=['GET'],)
    # def trigger_report_combined(request):
    #     report_ids = []

    #     csv_data = []
    #     # only triggering report for first 200 stores
    #     all_stores = Store.objects.all()[:200]
    ...
    #     generate_csv_file(report, csv_data)
    #     return Response(status=200, data={"report_id": report.ids})
